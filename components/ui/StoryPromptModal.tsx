...
export default function StoryPromptModal({
  ...
  useEffect(() => {
    if (!listening) {
      ...
      // Auto-restart if still in mic mode (browser stopped on its own)
      // Use ref to avoid stale closure — activeMode from state would be stale here
      if (activeModeRef.current === "mic" && browserSupportsSpeechRecognition) {
